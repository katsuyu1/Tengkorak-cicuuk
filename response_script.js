...
// Helpful runtime check — if THREE is missing it's usually because
// the page was opened using file:// or the dependency order is wrong.
if (typeof THREE === "undefined") {
  console.error(
    "Three.js (THREE) is not available. Make sure you run the page from a local HTTP server (e.g. VS Code Live Server) and that three.js is loaded before script.js."
  );
}
...
// Create small glowing 'fire' orbs orbiting the front card
function createFireOrbit(count = 12) {
  // Helper to make a radial glow canvas texture
  function makeGlowTexture(size = 256, inner = 'rgba(200,160,255,1)', mid = 'rgba(140,40,200,0.7)', outer = 'rgba(100,20,140,0)') {
  ...
  for (let i = 0; i < count; i++) {
    const baseR = 0.22 + Math.random()*0.1;
    const baseG = 0.00 + Math.random()*0.02;
    const baseB = 0.45 + Math.random()*0.15;

    // soft glow (sprite)
    const spriteMat = new THREE.SpriteMaterial({
      map: glowTex,
      color: new THREE.Color(baseR * 1.6, baseG * 0.8, baseB * 1.6),
      transparent: true,
      opacity: 0.9,
      depthWrite: false,
    ...
    sprite.scale.set(2.8 + Math.random()*1.8, 2.8 + Math.random()*1.8, 1);

    // inner core
    const coreMat = new THREE.MeshBasicMaterial({
      color: new THREE.Color(baseR*1.3, baseG*0.6, baseB*1.3),
      blending: THREE.AdditiveBlending,
      transparent: true,
      depthWrite: false
    });
    ...
    // holder groups sprites and core together
    const holder = new THREE.Object3D();
    holder.add(sprite);
    holder.add(core);
    holder.userData = {
      angleOffset: (Math.PI * 2 * i) / count + Math.random() * 0.5,
      radius: 5 + Math.random() * 2.5,
      heightOffset: (Math.random() - 0.5) * 3.0,
  ...
  // position the group around the frontcard
  scene.add(fireGroup);
  // increase bloom a little to emphasize glow
  if (bloomPass) bloomPass.strength = Math.max(bloomPass.strength, 1.2);
}
...
// update flames orbit around the card
function updateFireOrbit(timeMs) {
  if (!fireGroup || !frontcard) return;
  const t = timeMs * 0.001;
  for (let i = 0; i < fireGroup.children.length; i++) {
    const holder = fireGroup.children[i];
    const u = holder.userData;
    const angle = t * u.speed + u.angleOffset;
    const x = frontcard.position.x + Math.cos(angle) * u.radius;
    const z = frontcard.position.z + Math.sin(angle) * (u.radius * 0.7);
    const y = frontcard.position.y + u.heightOffset + Math.sin(angle * 2.0) * 0.7;
    holder.position.set(x, y, z);
    // flicker/animate sprite and core
    const flick = 0.65 + Math.abs(Math.sin(angle * 3.0));
    if (u.sprite) {
      // change sprite scale and opacity for pulsing glow
      const s = 1.6 + Math.abs(Math.sin(angle * 2.5)) * 1.6;
      u.sprite.scale.setScalar(s);
      u.sprite.material.opacity = 0.55 + Math.abs(Math.sin(angle * 2.0)) * 0.6;
    }
    if (u.core) {
      const cscale = 0.5 + Math.abs(Math.sin(angle * 4.0)) * 0.9;
      u.core.scale.setScalar(cscale);
      u.core.material.opacity = 0.6 + Math.abs(Math.cos(angle * 3.5)) * 0.6;
    }
  }
}
...
function animate(deltaTime) {
  ...
  updateFireOrbit(deltaTime || performance.now());
  composer.render();
